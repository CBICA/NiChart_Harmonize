...
def nh_harmonize_to_ref(model : Union[dict, str],
    ...
    for curr_batch in batch_col_vals:
        ...
        logger.info(len(ind_curr))
        logger.info(df_h_data_curr.isna().sum(axis=0))
        logger.info(df_h_data.isna().sum(axis=0))
        logger.info(ind_curr[0:10])
        logger.info(df_h_data.loc[ind_curr[0:10], 'MUSE_GM'])
        input('eee1')
